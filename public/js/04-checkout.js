...
// Skips SlickPay entirely and writes real purchase records straight to
// Firestore, exactly like a live order — so you can confirm products
// actually show up (and auto-deliver correctly) in "My Products" without
// waiting on a real card payment. Marked with paymentMethod:'test-simulated'
// and an order-notes flag so it's obvious in the admin panel which orders
// were test runs. Remove the button in the HTML above when you're done testing.
async function coSimulateTestPayment() {
  const currentUser = UserAuth.current();
  if (!currentUser) { openLoginModal('checkout'); closeCheckout(); return; }
  const cartItems = Cart.get();
  if (!cartItems.length) { closeCheckout(); return; }
  ...
  const name  = (document.getElementById('co-name')?.value || currentUser.name || 'Test Customer').trim();
  const email = (document.getElementById('co-email')?.value || currentUser.email || '').trim();
  const products = DB.getAll ? DB.getAll('products') : [];
  const orderId = 'TEST-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7).toUpperCase();

  const btn = document.getElementById('co-test-pay-btn');
    ...
    try {
      const purchase = await Purchases.add(
        ...
        },
        {
          customerName:  name,
          customerEmail: email,
          paymentMethod: 'test-simulated',
          orderNotes:    '⚠️ TEST PAYMENT — simulated, no real charge was made',
          orderId,
          variantLabel:  item.variantLabel || null,
        }
      );
      ...
      if (fullProd.autoDeliver && fullProd.deliveryLink) {
        // Product has auto-delivery configured — deliver it immediately,
        // exactly like a real completed payment would.
        const dtype = fullProd.deliveryType === 'pdf' ? 'pdf' : 'link';
        const accessData = { '_DeliveryType': dtype, 'Download Link': fullProd.deliveryLink };
        ...
        await Purchases.update(purchase.id, { accessData, accessLink: fullProd.deliveryLink, deliveryType: dtype });
      } else {
        // No auto-delivery configured on this product — still mark it paid
        // so you can see it land as "completed" and check the admin flow.
        await Purchases.update(purchase.id, { status: 'completed', deliveredAt: new Date().toISOString() });
      }
    } catch (err) {
      errors.push(item.name + ': ' + (err.message || String(err)));
  ...
  if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-flask"></i> Simulate Test Payment (Dev Only)'; }
